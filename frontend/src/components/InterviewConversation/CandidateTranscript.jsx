...
const CandidateTranscript = ({ transcript, transcriptState, onClearAnswer }) => {
  ...
  return (
    <div className="flex flex-col gap-3 px-2 h-full relative">
      {/* Listening Indicator (Absolute positioning to prevent layout shifts when it disappears) */}
      <div className={`absolute top-0 left-2 right-2 flex items-center gap-3 text-primary-400 font-medium transition-all duration-300 ${isListening ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-2 pointer-events-none'}`}>
      ...
      <div className={`min-h-[4rem] relative group flex-1 transition-all duration-500 pt-10 ${isListening || hasTranscript || isProcessing ? 'opacity-100' : 'opacity-0'}`}>
        {hasTranscript ? (
          ...
          <p className={`text-lg text-dark-400 font-medium opacity-80 mt-2 transition-opacity duration-300 ${isProcessing ? 'opacity-100' : 'opacity-0'}`}>
