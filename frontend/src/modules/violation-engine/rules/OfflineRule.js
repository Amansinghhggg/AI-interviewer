...
export class OfflineRule {
  constructor() {
    this.type = VIOLATION_TYPES.OFFLINE;
    this.source = VIOLATION_SOURCES.BROWSER;
  }

  evaluate({ event, activeViolations, configuration }) {
    if (!configuration.offlineEnabled) return null;

    if (event.event === BROWSER_EVENTS.NETWORK_OFFLINE) {
    ...
    if (event.event === BROWSER_EVENTS.NETWORK_ONLINE) {
      const active = activeViolations.find(v => v.type === this.type);
      if (active) {
        return {
          action: 'RESOLVE',
