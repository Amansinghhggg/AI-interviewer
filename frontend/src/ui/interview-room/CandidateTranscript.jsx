...
const CandidateTranscript = ({ transcript, transcriptState, onClearAnswer }) => {
  ...
  return (
    <div className="flex flex-col gap-3 px-2 h-full relative z-10">
      ...
      <div className={`absolute top-0 left-2 right-2 flex items-center gap-3 text-[var(--primary)] font-bold transition-all duration-300 ${isListening ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-2 pointer-events-none'}`}>
        ...
        <div className="flex items-end gap-1.5 h-4 ml-1">
          {[...Array(3)].map((_, i) => (
            <div
              key={i}
              className="w-1.5 bg-[var(--primary)] rounded-full animate-waveform-bar shadow-sm"
      ...
      <div className={`min-h-[4rem] relative group flex-1 transition-all duration-500 pt-10 ${isListening || hasTranscript || isProcessing ? 'opacity-100' : 'opacity-0'}`}>
        {hasTranscript ? (
          <div className="relative bg-[var(--background-secondary)] p-6 rounded-2xl border border-[var(--border)] shadow-lg hover:border-[var(--primary)] transition-colors group/transcript">
            <p className="text-xl leading-relaxed text-[var(--text-primary)] pr-10 font-medium">
              {transcript}
              {isListening && (
                <span className="inline-block w-1 h-6 bg-[var(--primary)] ml-2 align-text-bottom animate-cursor-blink shadow-sm" />
              )}
            </p>
            {!isListening && onClearAnswer && (
              <button 
                onClick={onClearAnswer}
                className="absolute top-4 right-4 p-2 text-[var(--text-secondary)] hover:text-[var(--color-danger)] hover:bg-[var(--color-danger)]/10 rounded-lg transition-all opacity-0 group-hover/transcript:opacity-100"
          ...
          <p className={`text-lg text-[var(--text-secondary)] font-bold uppercase tracking-wider opacity-80 mt-4 transition-opacity duration-300 ${isProcessing ? 'opacity-100' : 'opacity-0'}`}>
